...
class MFASetupView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: {"type": "object", "properties": {"qr_code": {"type": "string"}, "secret": {"type": "string"}}},
    ...
    def get(self, request):
        ...
        otp_uri = pyotp_instance.provisioning_uri(name=request.user.email, issuer_name="Larvixon")
...
class MFAVerifyView(APIView):
    ...
    def post(self, request):
        ...
        is_valid, error_message, device = verify_mfa_code(request.user, code, is_confirmed_check=False)

        if is_valid:
            if not device.last_used_at:
                device.last_used_at = timezone.now()
                device.save()
                return Response({"detail": "MFA successfully activated."}, status=status.HTTP_200_OK)
            else:
                return Response({"detail": "MFA device is already confirmed."}, status=status.HTTP_400_BAD_REQUEST)
        else:
...
class MFADeactivateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            device = Authenticator.objects.get(user=request.user, type=Authenticator.Type.TOTP)
            device.delete()
        except Authenticator.DoesNotExist:
            pass # No device to delete

        return Response({"detail": "MFA successfully deactivated."}, status=status.HTTP_200_OK)
