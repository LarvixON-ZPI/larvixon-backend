...
class TestVideoAnalysis(APITestCase):
    ...
    def test_01_create_analysis(self):
        ...
        response = self.make_request("POST", "/analysis/", create_data)
        self.assertEqual(
            response.status_code, 201, f"Create analysis failed: {response.text}"
        )
    ...
    def test_02_get_analyses_list(self):
        ...
        self.assertEqual(
            response.status_code, 200, f"Get analyses failed: {response.text}"
        )
    ...
    def test_03_update_analysis_feedback(self):
        ...
        response = self.make_request("PATCH", f"/analysis/{analysis_id}/", update_data)
        self.assertEqual(
            response.status_code, 200, f"Update feedback failed: {response.text}"
        )
